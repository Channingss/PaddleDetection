...
cv::Mat VisualizeResult(const cv::Mat& img,
  ...
  for (int i = 0; i < results.size(); ++i) {
    ...
    cv::Rect roi = cv::Rect((int)results[i].rect[0],(int)results[i].rect[2], w, h);
...
void ObjectDetector::Postprocess(
  ...
  for (int j = 0; j < total_size; ++j) {
    ...
    float xmin = (output_data_[2 + j * 6] * rw);
    float ymin = (output_data_[3 + j * 6] * rh);
    float xmax = (output_data_[4 + j * 6] * rw);
    float ymax = (output_data_[5 + j * 6] * rh);
    int wd = xmax - xmin;
    int hd = ymax - ymin;
    if (score > threshold_) {
